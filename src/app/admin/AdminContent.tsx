  ...
  const [portfolios, setPortfolios] = useState([
    {
      id: 1,
      title: '체중 감량 성공 사례',
      description: '3개월 만에 15kg 감량 성공',
      imageUrl: '/portfolio/before-after-1.jpg',
      category: '다이어트',
      date: '2024-01-15'
    },
    {
      id: 2,
      title: '자세교정 성공 사례',
      description: '거북목 자세교정 성공',
      imageUrl: '/portfolio/posture-1.jpg',
      category: '자세교정',
      date: '2024-01-10'
    }
  ])
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {portfolios.map(portfolio => (
            <div key={portfolio.id} className="border rounded-lg overflow-hidden">
              ...
              <div className="p-4">
                <h4 className="font-semibold text-lg mb-1">{portfolio.title}</h4>
                <p className="text-gray-600 text-sm mb-2">{portfolio.description}</p>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-blue-600">{portfolio.category}</span>
                  <span className="text-sm text-gray-500">{portfolio.date}</span>
                </div>
                <div className="mt-4 flex space-x-2">
                  <button
                    onClick={() => alert('포트폴리오를 수정합니다.')}
                    className="px-3 py-1 text-sm text-blue-600 border border-blue-600 rounded hover:bg-blue-50"
                  >
                    수정
                  </button>
                  <button
                    onClick={() => alert('포트폴리오를 삭제합니다.')}
